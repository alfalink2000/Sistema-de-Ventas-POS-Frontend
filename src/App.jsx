...
function AppContent() {
  ...
  useEffect(() => {
    const initializeAppSafely = async () => {
      try {
        ...
        // 1. Inicializar IndexedDB para funcionalidad offline
        await IndexedDBService.init();
  ...
  // ✅ Verificar autenticación y disponibilidad offline
  useEffect(() => {
    if (isAuthenticated && user) {
      ...
      const checkOfflineData = async () => {
        try {
          // Verificar token para acceso offline
          const token = localStorage.getItem("token");
          if (!token) {
          ...
          console.log("✅ Token disponible para acceso offline");

          // Aquí puedes agregar lógica para verificar datos pendientes
          // sin los controladores complejos
        } catch (error) {
